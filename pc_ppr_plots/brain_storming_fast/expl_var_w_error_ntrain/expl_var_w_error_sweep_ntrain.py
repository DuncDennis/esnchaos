...
sweep_dict = {
    "t_train": dict(
        sweep_name=r"N_\text{T}",
        sweep_values=[200, 300, 400, 500, 600, 1000, 2000, 3000],
        f_name="t_train",
        # full_val_range=[200, 1000],
        full_val_range=None,
        log_val_bool=False,
    ),
    "n_rad": dict(
...
for sweep_key in ["t_train"]:
    ...
    # RC build args:
    build_args_cp = build_args.copy()

    # data creation args:
    ts_creation_args_cp = ts_creation_args.copy()

    for i_sweep, sweep_value in enumerate(sweep_values):

        if sweep_key in build_args_cp.keys():
            build_args_cp[sweep_key] = sweep_value
        elif sweep_key in ts_creation_args_cp.keys():
            ts_creation_args_cp[sweep_key] = sweep_value
        else:
            raise ValueError(f"sweep_key __{sweep_key}__ must be either in build_args or in "
                             "ts_creation_args. ")

        n_train = ts_creation_args_cp["n_train_sects"]
        train_sync_steps = ts_creation_args_cp["t_train_sync"]
        train_data_list, validate_data_list_of_lists = sweep.time_series_creator(
            sys_obj,
            **ts_creation_args_cp)
    ...
    for i_sweep, sweep_value in enumerate(sweep_values):
        ...
        # Note: sweep_value is t_train here.
        ev_cutoff = build_args_cp["reg_param"] / sweep_value

        # add hline for factor plot:
        fig.add_trace(
            go.Scatter(x=xrange,
                       y=[ev_cutoff, ev_cutoff],
                       line=dict(color=rgb_line, dash="dash", width=3),
                       marker=dict(size=0),
                       showlegend=False,
                       mode="lines",
                       ),
