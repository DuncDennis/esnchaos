...
HYBRIDNAMES = {"no_hybrid": "reservoir-only",
               "input_hybrid": "input hybrid",
               "output_hybrid": "output hybrid",
               "full_hybrid": "full hybrid",
               "model_predictor": "KBM-only",
               "model_predictor_fitted": "KBM-fitted"}

# NAMES FOR SYSTEMS:
def rename_systems(x: str):
...
if __name__ == "__main__":
    ...
    for i, pkl_file_path in enumerate(pkl_files):
        ...
        if "P system" in param_cols:
            x_param = "P system"
            no_xaxis_title=True
            df_agg["P system"] = df_agg["P system"].apply(rename_systems)
        ...
        elif out_plot_name.startswith("ks_eps_model_effect_of_eps"):
            x_axis_dict = {"dtick": 1}
            y_axis_dict = Y_AXIS_DICT

        # elif out_plot_name.startswith("all_systems_large_res_good_eps") or out_plot_name.startswith("all_systems_small_res_good_eps"):
        #     x_axis_dict = X_AXIS_DICT
        #     y_axis_dict = {"range": [-1, 17]}
        else:
            x_axis_dict = X_AXIS_DICT
            y_axis_dict = Y_AXIS_DICT
        ...
        fig.write_image(f"{OUTPUT_DIR}/{out_plot_name}.png", scale=4)
