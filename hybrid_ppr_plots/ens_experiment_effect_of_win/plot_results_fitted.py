...
if __name__ == "__main__":
    ...
    for i, pkl_file_path in enumerate(pkl_files):
        ...
        # only win:
        w_in_list = [0.1, 0.5, 1.0, 3]
        for w_in in w_in_list:
            w_in_list_exclude = w_in_list.copy()
            w_in_list_exclude.remove(w_in)

            df_pre_fil = pkl_to_df.pre_filter_df(df, excluded_params={"P w_in_scale": w_in_list_exclude},
                                    rmv_const_cols=True)
            ...
            fig.write_image(f"{OUTPUT_DIR}/{out_plot_name}_{str(w_in)}.png", scale=3)
