"""
REDO THE LARGE RES GOOD EPS EXPERIMENT WITH:
- partly new dt's for better lyapunov values.
- the newly calculated lyapunov values (with the new dts) (not taken from sprott anymore).
    - Apart from rucklidge the calculated values are only 5% different to the sprott value.
"""
...
import esnchaos.rom as rom

name_of_file = "all_systems_large_res_ROM_new_dts"
...
def parameter_transformer(parameters: dict[str, float | int | str]):
    ...
    time_series_rom = sys_obj.simulate(1000 + 5000)[1000:, :]

    # just take the first train data for now (not really good though).
    _, _, wrong_model = rom.get_rom(time_series_rom, sys_obj, n_pod=2)
