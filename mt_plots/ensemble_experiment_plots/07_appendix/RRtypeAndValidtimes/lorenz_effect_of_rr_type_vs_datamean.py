...
name_of_file = "lorenz_effect_of_rr_type_vs_datamean"
...
parameters = {
    ...
    # Meta params:
    "rr_type": ["a", "b", "c"],  # sets "r_to_rgen_opt", and "ridge_regression_opt".
    # "data_offset": [0, 10, 20, 30, 50, 100, 500, 1000],
    # "data_offset": [0, 50, 100, 150, 200, 250, 300, 350, 400],
    "data_offset": [0, 100, 200, 300, 400, 500, 600],
...
def parameter_transformer(parameters: dict[str, float | int | str]):
    ...
    if p["rr_type"] == "a":
        ...
        p["ridge_regression_opt"] = "no_bias"
    elif p["rr_type"] == "b":
        p["r_to_rgen_opt"] = "linear_r"
        p["ridge_regression_opt"] = "bias"
    elif p["rr_type"] == "c":
        p["r_to_rgen_opt"] = "output_bias"
        p["ridge_regression_opt"] = "no_bias"
    else:
        raise ValueError("rr_type not accounted for. ")
    ...
    # Add data_offset:
    train_data_list = [x + p["data_offset"] for x in train_data_list]
    validate_data_list_of_lists_new = []
    for validate_data_list in validate_data_list_of_lists:
        validate_data_list_of_lists_new.append([x + p["data_offset"] for x in validate_data_list])
    ...
    train_validate_test_args = {
        ...
        "train_metrics": {"Dummy": sweep.dummy_metric}, # no train metrics needed for faster simulation
        "validate_metrics": {"VT": sweep.valid_time,
                             "VT_centered": sweep.valid_time_centered_error},
        "opt_validate_metrics_args": {"VT":
                                          {"dt": p["dt"],
                                           "lle": p["lle"]},
                                      "VT_centered":
                                          {"dt": p["dt"],
