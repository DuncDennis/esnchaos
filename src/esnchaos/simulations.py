...
class ComplexButterfly(_SimBaseRungeKutta):
...
class ChuaCircuit(_SimBaseRungeKutta):
    ...
    def __init__(
        ...
        Args:
            ...
            dt: Size of time steps.
        """
        self.alpha = self.default_parameters["alpha"] if alpha is None else alpha
        self.beta = self.default_parameters["beta"] if beta is None else beta
        self.a = self.default_parameters["a"] if a is None else a
        self.b = self.default_parameters["b"] if b is None else b
        self.dt = self.default_parameters["dt"] if dt is None else dt
...
class Rucklidge(_SimBaseRungeKutta):
    ...
    def __init__(self, kappa: float | None = None, lam: float | None = None, dt: float | None = None) -> None:
        ...
        """
        self.kappa = self.default_parameters["kappa"] if kappa is None else kappa
        self.lam = self.default_parameters["lam"] if lam is None else lam
        self.dt = self.default_parameters["dt"] if dt is None else dt
...
class Halvorsen(_SimBaseRungeKutta):
...
class LotkaVolterra(_SimBaseRungeKutta):
    ...
    def __init__(
        ...
        self.b = self.default_parameters["b"] if b is None else b
        self.c = self.default_parameters["c"] if c is None else c
        self.d = self.default_parameters["d"] if d is None else d
        self.dt = self.default_parameters["dt"] if dt is None else dt
...
class Logistic(_SimBase):
    ...
    def __init__(self, r: float | None = None) -> None:
        ...
        """
        self.r = self.default_parameters["r"] if r is None else r
...
class SimplestDrivenChaotic(_SimBaseRungeKutta):
    ...
    def __init__(self, omega: float | None = None, dt: float | None = None) -> None:
        ...
        Args:
            ...
            dt: Size of time steps.
        """
        self.omega = self.default_parameters["omega"] if omega is None else omega
        self.dt = self.default_parameters["dt"] if dt is None else dt
...
class KuramotoSivashinsky(_SimBase):
    ...
    def __init__(
        ...
        self.sys_dim = self.default_parameters["sys_dim"] if sys_dim is None else sys_dim
        self.sys_length = self.default_parameters["sys_length"] if sys_length is None else sys_length
        self.eps = self.default_parameters["eps"] if eps is None else eps
        self.dt = self.default_parameters["dt"] if dt is None else dt
...
class KuramotoSivashinskyCustom(_SimBase):
    ...
    def __init__(
        ...
        self.sys_length = self.default_parameters["sys_length"] if sys_length is None else sys_length
        self.dt = self.default_parameters["dt"] if dt is None else dt
        self.precision = self.default_parameters["precision"] if precision is None else precision
        self.fft_type = self.default_parameters["fft_type"] if fft_type is None else fft_type
...
class Lorenz96(_SimBaseRungeKutta):
    ...
    def __init__(self, sys_dim: int | None = None, force: float | None = None, dt: float | None = None) -> None:
        ...
        """
        self.sys_dim = self.default_parameters["sys_dim"] if sys_dim is None else sys_dim
        self.force = self.default_parameters["force"] if force is None else force
        self.dt = self.default_parameters["dt"] if dt is None else dt
