...
def lyapunov_exponent_spectrum(
    ...
    """Numerically calculate the spectrum of m largest lyapunov exponent given an iterator function.

    A mixture of:
    - The algorithm for the largest lyapunov exponent: Sprott, Julien Clinton, and Julien C. Sprott. Chaos and
        time-series analysis. Vol. 69. Oxford: Oxford university press, 2003.
    - The algorithm for the spectrum given in 1902.09651 "LYAPUNOV EXPONENTS of the KURAMOTO-SIVASHINSKY PDE".
    ...
    Args:
        ...
        initial_pert_directions:
            - If np.ndarray: The direction of the initial perturbations of shape (m, x_dim). Assumed to be orthogonal
                             and normalized.
    ...
    Returns:
        The Lyapunov exponent spectrum of largest m values. If return_convergence is True: The convergence
        (2D N x m np.ndarray), else a (1D m-size np.ndarray), which holds the last values in the convergence.
    """
    ...
    if initial_pert_directions is None:
        initial_perturbations = np.eye(x_dim)[:m, :] * deviation_scale  # each vector is of length deviation_scale
    ...
    for i_n in range(steps + steps_skip):
        ...
        if i_n >= steps_skip:
            log_divergence_spec[i_n - steps_skip, :] = np.log(np.abs(np.diag(r)) / deviation_scale)

    if return_convergence:
        return np.array(np.cumsum(log_divergence_spec, axis=0) / (
                    np.repeat(np.arange(1, steps + 1)[:, np.newaxis], m, axis=1) * dt * part_time_steps))
    else:
