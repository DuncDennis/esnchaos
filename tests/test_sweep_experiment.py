...
import esnchaos.plot.sweep_plots as sp
...
def test_prefilter_df(results_df):
    ...
    excl_params_df = sp.pre_filter_df(results_df,
...
def test_data_aggregation(results_df):
    ...
    df_agg_mean = sp.aggregate_df(results_df,
                             avg_mode="mean_and_std")
    df_agg_median = sp.aggregate_df(results_df,
                                    avg_mode="median_and_quartile")

    # check columns:
    orig_metric_cols = sp._get_metric_cols(results_df)
    new_metric_cols = sp._get_metric_cols(df_agg_mean)
    ...
    # check output size:
    parameter_cols = sp._get_param_cols(results_df)
    ...
    df_agg_metrics_sub = sp.aggregate_df(results_df,
                                         metrics_subset=["M VALIDATE MSE"])

    metric_cols_subset = sp._get_metric_cols(df_agg_metrics_sub)
